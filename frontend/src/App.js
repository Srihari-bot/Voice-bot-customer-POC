...
function App() {
  ...
  const handleVoiceInput = async (text) => {
    ...
    console.log('\n🎤 ========================================');
    console.log('🎤 NEW VOICE INPUT REQUEST');
    console.log('🎤 ========================================');
    console.log(`📝 User Input: "${text}"`);
    console.log(`⏱️  Request Start Time: ${new Date().toLocaleTimeString()}.${Math.floor(performance.now() % 1000)}`);
    ...
    try {
      ...
      console.log('⏱️  [1] Sending /chat request to backend...');
      ...
      console.log(`⏱️  [1] /chat response received: ${chatDuration}s`);
      ...
      if (response.data.success) {
        ...
        console.log(`⏱️  Time before TTS call: ${chatToTTSDelay}s`);
        ...
        if (aiResponseText && aiResponseText.trim().length > 0) {
          ...
          console.log('⏱️  [2] Preparing TTS request...');
          console.log('🔊 Text length:', fullTextForTTS.length, 'characters');
          console.log('🔊 Language:', detectedLanguageFromBackend);
  ...
  const speakText = async (text, preferredLanguage = null, requestStartTime = null) => {
    ...
    try {
      ...
      console.log('⏱️  [2] Sending /tts request to backend...');
      ...
      console.log(`⏱️  [2] /tts response received: ${ttsApiDuration}s`);
      ...
      if (audioElement.duration && !isNaN(audioElement.duration) && isFinite(audioElement.duration)) {
        ...
        console.log(`🔊 Using actual audio duration: ${audioElement.duration.toFixed(2)}s, timeout: ${(timeoutDuration / 1000).toFixed(1)}s (NO LIMIT)`);
      } else {
        ...
        console.log(`🔊 Estimated audio duration: ${(estimatedDuration / 1000).toFixed(1)}s, timeout: ${(timeoutDuration / 1000).toFixed(1)}s (NO LIMIT)`);
      }
      ...
      fallbackTimeout = setTimeout(() => {
        console.log('⚠️ TTS safety timeout reached - but audio will continue playing until complete');
        ...
        if (audioElement && !audioElement.ended) {
          const progress = audioElement.duration > 0 
            ...
            : 'unknown';
          console.log(`⚠️ Audio still playing: ${audioElement.currentTime.toFixed(2)}s / ${audioElement.duration.toFixed(2)}s (${progress}% complete)`);
          console.log('⚠️ Audio will continue playing - no interruption');
      ...
      audioElement.onended = () => {
        ...
        console.log(`✅ Played until: ${finalTime.toFixed(2)}s (${completionPercentage.toFixed(1)}% complete)`);
        ...
        if (finalTime >= finalDuration * 0.99) {
          console.log(`✅ FULL TEXT WAS SPOKEN - NO TRUNCATION (${completionPercentage.toFixed(1)}% complete)`);
        } else {
          console.warn(`⚠️ Audio may not have played fully: ${completionPercentage.toFixed(1)}% complete`);
        }
      ...
      audioElement.addEventListener('timeupdate', () => {
        if (audioElement.duration && !isNaN(audioElement.duration) && isFinite(audioElement.duration)) {
          ...
          if (Math.floor(progress / 10) > Math.floor(lastLoggedProgress / 10)) {
            console.log(`🔊 TTS playing: ${Math.floor(progress / 10) * 10}% complete (${remaining.toFixed(1)}s remaining, ${audioElement.currentTime.toFixed(1)}s / ${audioElement.duration.toFixed(1)}s)`);
            lastLoggedProgress = progress;
          }
          
          // Log when near completion (95%+)
          if (progress >= 95 && lastLoggedProgress < 95) {
            console.log(`🔊 TTS near completion: ${progress.toFixed(1)}% complete (${remaining.toFixed(1)}s remaining)`);
            lastLoggedProgress = progress;
          }
        }
      });
      ...
      if (audioElement.duration && !isNaN(audioElement.duration) && isFinite(audioElement.duration)) {
        ...
        if (audioElement.duration < expectedDuration * 0.7) {
          ...
          console.warn(`⚠️ Expected: ${expectedDuration.toFixed(2)}s, Actual: ${audioElement.duration.toFixed(2)}s`);
        } else {
          console.log('✅ Audio duration matches expected length - full text should be spoken');
        }
      }
      ...
      console.log(`⏱️  [3] Audio playback started: ${playTime}s after TTS call`);
      ...
      if (requestStartTime) {
        ...
        console.log('✅ ========================================');
        console.log(`⏱️  User Input → Audio Playing: ${totalTime}s`);
        console.log(`🎯 Target: 2.0s | Actual: ${totalTime}s`);
        if (parseFloat(totalTime) <= 2.0) {
          console.log('🎉 TARGET ACHIEVED! ✅');
        } else if (parseFloat(totalTime) <= 3.0) {
          console.log('⚡ Very Good! Close to target');
        } else {
